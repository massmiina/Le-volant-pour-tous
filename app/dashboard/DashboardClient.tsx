        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
          <div>
            <h1 className={`text-3xl md:text-5xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-violet-400 to-emerald-400 ${rockSalt.className}`}>
              Bonjour, {user.name || "Pilote"} 🚀
            </h1>
            <p className="text-gray-400 mt-4 text-lg">
              Prêt pour votre prochaine session d'entraînement ?
            </p>
          </div>
          ...
          <div className="flex flex-col items-center">
            ...
            <span className="text-sm text-gray-500 mt-3 uppercase tracking-widest">Complété</span>
          </div>
        </div>
      </motion.section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        >
           <div className="bg-white/5 border border-white/10 rounded-2xl p-6 relative overflow-hidden group">
              ...
              <h2 className="text-xl font-bold text-white mb-4 relative z-10">Reprendre l'apprentissage</h2>
        ...
        >
          <div className="bg-white/5 border border-white/10 rounded-2xl p-6">
             <h2 className="text-xl font-bold text-white mb-4">Derniers Examens</h2>
             
             {examResults.length === 0 ? (
                <p className="text-gray-400 text-sm">Aucun examen passé pour l'instant.</p>
             ) : (
                <div className="space-y-3">
                  {examResults.map((exam, idx) => (
                    <div key={idx} className={`flex justify-between items-center p-4 bg-black/40 rounded-xl border ${exam.passed ? 'border-emerald-500/30' : 'border-red-500/30'}`}>
                      <span className="text-gray-300">Test Blanc #{examResults.length - idx}</span>
