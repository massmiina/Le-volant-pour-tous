...
import SignLexicon from '@/components/SignLexicon';

export default function CoursSignalisation() {
  const { t, language } = useLanguage();

  // Get data from translations
  // Note: Since translations.ts structure is complex, we target signalisation directly
  const signalisationData = (t('cours.signalisation', { returnObjects: true }) as any);
  
  // If useLanguage's t doesn't support returnObjects (custom implementation), we handle it
  // Our custom t returns the key if not found or the string if found.
  // Actually, I'll map manually to be safe since I know the structure.
  
  const sections = signalisationData.sections || [];
  const quizData = signalisationData.quiz_section || { title: "", intro: "", questions: [] };
  ...
  return (
    <div className="bg-gray-50 min-h-screen pb-24 font-sans">
      ...
      <div className="max-w-4xl mx-auto px-4 mt-12 space-y-16">
        ...
        {/* Lexique Visuel Complet des Panneaux */}
        <AnimatedSection>
          <SignLexicon />
        </AnimatedSection>
        ...
        <AnimatedSection>
           <ModuleQuiz moduleId={1} questions={quizData.questions} title={quizData.title} intro={quizData.intro} />
        </AnimatedSection>

        {/* Footer Navigation */}
        <AnimatedSection>
          <div className="bg-gray-900 rounded-3xl p-10 md:p-14 text-center shadow-xl mt-8">
            <h2 className="text-3xl font-black text-white mb-4 tracking-tight">{t('cours.signalisation.footer_title')}</h2>
            <p className="text-gray-400 font-bold mb-8 text-xl">
              {t('cours.signalisation.footer_desc')}
            </p>
            <div className="flex flex-col sm:flex-row justify-center items-center gap-6">
              <Link href="/cours/2" className="w-full sm:w-auto px-10 py-5 bg-blue-600 hover:bg-blue-500 text-white font-black rounded-xl transition-all shadow-lg hover:shadow-blue-900/40 text-lg flex items-center justify-center gap-2">
                {t('ui.next_module', { number: 2 })}
